...
test "Trigger: first note at frame=0" {
    ...
    testAll(&trigger, Notes(f32).ImpulsesAndParamses {
        ...
        ExpectedResult { .start = 0, .end = 1024, .params = 440.0, .note_id_changed = true },
    });
}
...
test "Trigger: first note after frame=0" {
    ...
    testAll(&trigger, Notes(f32).ImpulsesAndParamses {
        ...
        ExpectedResult { .start = 500, .end = 1024, .params = 440.0, .note_id_changed = true },
    });
}
...
test "Trigger: carryover" {
    ...
    testAll(&trigger, Notes(f32).ImpulsesAndParamses {
        ...
        ExpectedResult { .start =   0, .end =  200, .params = 440.0, .note_id_changed = true },
        ExpectedResult { .start = 200, .end = 1024, .params = 220.0, .note_id_changed = true },
    });
    ...
    testAll(&trigger, Notes(f32).ImpulsesAndParamses {
        ...
        ExpectedResult { .start =   0, .end =  500, .params = 220.0, .note_id_changed = false },
        ExpectedResult { .start = 500, .end =  600, .params = 330.0, .note_id_changed = true },
        ExpectedResult { .start = 600, .end = 1024, .params = 660.0, .note_id_changed = false },
    });
    ...
    testAll(&trigger, Notes(f32).ImpulsesAndParamses {
        ...
        ExpectedResult { .start = 0, .end = 1024, .params = 660.0, .note_id_changed = false },
    });
}
